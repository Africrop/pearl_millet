...
#### Load posteriors predictions
load("./nn3_tol005_default")
# load("~/Mil_Splatche/FINAL/nn3_tol005_default")
...
for (n in 1:nrow(param)){
  ...
  for(r in 1:10){
    ...
    alpha <- sample(nn3$adj.values[which(nn3$adj.values[,"alpha"]>0),"alpha"],1) ; names(alpha)<-"alpha"
    ...
    beta <- sample(nn3$adj.values[which(nn3$adj.values[,"beta"]>0),"beta"],1) ; names(beta)<-"beta"
    ...
    gamma <- sample(nn3$adj.values[which(nn3$adj.values[,"gamma"]>0),"gamma"],1) ; names(gamma)<-"gamma"
