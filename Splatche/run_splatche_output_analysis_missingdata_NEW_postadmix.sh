...
scp -rp nas:/$path_to_dir/splatche_results_cluster_analysis_missingdata_NEW_postadmix.R $path_to_tmp/
scp -rp nas:/$path_to_dir/mixture_for_splatche_dev.R $path_to_tmp/
scp -rp nas:/$path_to_data/param* $path_to_tmp/
scp -rp nas:/$path_to_dir/admix_posteriors_FINAL.txt $path_to_tmp/
...
if [ $# != 0 ]
	...
	then
		/usr/local/bin/Rscript $path_to_tmp/splatche_results_cluster_analysis_missingdata_NEW_postadmix.R $1 $2
fi

##### Transfert des données du noeud vers master apres effacement des dossiers compresses
rm -rf $path_to_tmp/*.tar.gz
rcp -rp $path_to_tmp/ nas:/$path_to_dir/postadmix/
