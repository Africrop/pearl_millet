...
#### Defining variables ####
gen_min <- 400 ; gen_max <- 1300 # min and max generations number
acc_min <- 0.05 ; acc_max <- 0.5 # min and max growing rate
mig_min <- 0.05 ; mig_max <- 0.75 # min and max migration rate
res_min <- 100 ; res_max <- 10000 #min and max resize parameter
resB_min <- 500 ; resB_max <- 50000 #min and max resizeB parameter
Tres_min <- 1000 ; Tres_max <- 4000 #min and max duration of bottleneck parameter
TrecBott_min <- 0 ; TrecBott_max <- 0 # min and max time in generations for the recent bottleneck
MutRate_prior <- 0.00001 #prior for mutation rate
K_min <- 100 ; K_max <- 500 # min and max carrying capacity for each cluster
...
splatche = function(input = "settings.txt",
  ...
  ZZ[3] = paste(TrecBott," ./datasets_1layer/veg2K_pop1_time_2.txt",sep="")
  ...
  CC = scan(file = "./datasets_1layer/veg2K_pop1_time_2.txt",
            ...
            nlines = 0,
            comment.char = "")
  CC[3] <- paste("3\t",round(K),"\tTropical woodland",sep="")
  CC[4] <- paste("4\t",round(K),"\tTropical thorn scrub and scrub woodland",sep="")
  CC[5] <- paste("5\t",round(K),"\tTropical semi-desert",sep="")
  CC[6] <- paste("6\t",round(K),"\tTropical grassland",sep="")
  CC[8] <- paste("8\t",round(K),"\tSavanna",sep="")
  CC[9] <- paste("9\t",round(K),"\tBroadleaved temperate evergreen forest",sep="")
  CC[12] <- paste("12\t",round(K),"\tTemperate deciduous broadleaved forest",sep="")
...
#### Charge la carte de l'Afrique ####
africa = read.table("ppveg.asc", skip = 6)

#### Creation d'une fonction de test des coordonnees tirees pour savoir si elles ne sont pas en dehors de la zone definie ####
