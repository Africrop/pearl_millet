...
#### Load admixture parameters ####
admix_coef <- read.table("admix_posteriors_FINAL.txt")
...
for (n in 1:nrow(param)){
  ...
  for(r in 1:10){
    ...
    alpha <- sample(admix_coef$alpha[which(admix_coef$alpha<1&admix_coef$alpha>0)],1) ; names(alpha)<-"alpha"
    ...
    beta <- sample(admix_coef$beta[which(admix_coef$beta<1&admix_coef$beta>0)],1) ; names(beta)<-"beta"
    ...
    gamma <- sample(admix_coef$gamma[which(admix_coef$gamma<1&admix_coef$gamma>0)],1) ; names(gamma)<-"gamma"
...
write.table(file = paste(args[2],"sum.stat_postadmix.txt",sep=""), sum.stat, row.names = F, quote=F)
}
print("saving computed statistics to file")
write.table(file = paste(args[2],"sum.stat_postadmix.txt",sep=""), sum.stat, row.names = F, quote=F)
